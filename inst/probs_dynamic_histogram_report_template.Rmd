...
fake_data_for_dynamic_probs_hist <- nested_data_frame_with_probs_distributions |> 
  purrr::map(\(reference_group){
    reference_group |> 
      purrr::map(\(stratified_by_value){
        stratified_by_value |> 
          purrr::pluck("probs_distribution")
        
        })})

full_hist_dat_list <- fake_data_for_dynamic_probs_hist |> 
  purrr::map(\(reference_group){
    reference_group |> 
      purrr::map(
        \(stratified_by_value){
          stratified_by_value |>
            purrr::map(create_full_hist_dat)
          }
        )
      }
    )

saveRDS(full_hist_dat_list, "full_hist_dat_list.rds")

    
dynamic_hist_predicted <- r2d3::r2d3(
  data = data_to_json(full_hist_dat_list),
  script = "dynamic_hist_predictions.js",
  width = 350,
  height = 350,
  options = list(
    by = params$by
...
#   create_predictions_cutoff_filter(0.01),
#   time_horizon_slider(
#     "confusion_matrix_for_display",
...
time_horizon_slider(
  ...
  "Time Horizon",
  0,
  length(params$fixed_horizon_times),
  step = 1
...
crosstalk::bscols(
    htmltools::div(dynamic_hist_predicted,
