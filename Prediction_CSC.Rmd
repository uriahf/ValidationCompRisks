...
rsel$dt <- 1
vsel$dt <- 2
cdata <- base::rbind(rsel, vsel)
cdata$dt <- base::factor(
  cdata$dt,
  levels = c(1, 2),
  labels = c("Development data", "Validation data")
)
Hmisc::label(cdata$age) <- "Age"
Hmisc::label(cdata$size) <- "Size"
Hmisc::label(cdata$ncat) <- "Nodal status"
Hmisc::label(cdata$hr_status) <- "Hormon receptor status"
base::units(cdata$age) <- "years"
base::units(cdata$size) <- "cm"
```
...
res_ci <- base::round(res_ci, 2)
base::rownames(res_ci) <- c("1-year", 
                            "2-year", 
                            "3-year", 
                            "4-year", 
                            "5-year")

base::colnames(res_ci) <- rep(c("Estimate", 
                                "Lower .95", 
                                "Upper .95"), 2)
knitr::kable(res_ci, 
             row.names = TRUE) %>%
...
res_AIC <- matrix(c(
  stats::AIC(fit_csc1), 
  stats::AIC(fit_csc1_rcs),
  stats::AIC(fit_csc2), 
  stats::AIC(fit_csc2_rcs)
),
byrow = T,
ncol = 2,
nrow = 2,
...
We calculate calibration plot and numerical summaries of calibration as ICI, E50, E90, Emax and root squared bias using pseudo value approach.
...
res_discr_csh <- base::round(res_discr_csh, k)

knitr::kable(res_discr_csh) %>%
  kableExtra::kable_styling("striped", 
                            position = "center") %>%
...
res_ov <- base::round(res_ov_csh, 2) # Digits
knitr::kable(res_ov) %>%
  kableExtra::kable_styling("striped", 
                            position = "center") |>
  kableExtra::add_header_above(c(
